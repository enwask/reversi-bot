...
position *teamrandMove(const enum piece board[][SIZE], enum piece mine, int secondsleft) {
    // Translate stuff
    board_t state = teamrand_loadBoard(board);
    int color = (mine == WHITE);
    
    // Perform the move with our own state format
    pos_t pos = teamrand_getMove(state, color, secondsleft);
 ...
 */
pos_t teamrand_getMove(board_t state, int color, int tim) {
    //teamrand_print(state);
    //printf("\nBlack has %d pieces\nWhite has %d pieces\n\n", teamrand_count(state, 0), teamrand_count(state, 1));
    solvePair_t pair = teamrand_solveBoard(state, color, 1);
    return pair.pos;
}
...
int teamrand_evalBoard(board_t state, int color) {
    return rand();
}
...
solvePair_t teamrand_solveBoard(board_t state, int color, int layer) {
    if (layer == 0) {
        int score = teamrand_evalBoard(state, color);
        pos_t pos = teamrand_makePos(-1, -1);
        return teamrand_makeSolvePair(pos, score);
    }

    int best = -1e9;
    pos_t bestPos = teamrand_makePos(0, 0);
    int iCanPlay = 0;
    int otherCanPlay = 0;
    for (int r = 0; r < 8; r++) {
        for (int c = 0; c < 8; c++) {
            pos_t pos = teamrand_makePos(r, c);
            if (!teamrand_boardEquals(state, teamrand_executeMove(state, pos, color^1))) otherCanPlay = 1;
            board_t nState = teamrand_executeMove(state, pos, color);
            if (teamrand_boardEquals(state, nState)) continue;
            iCanPlay = 1;
            solvePair_t pair = teamrand_solveBoard(nState, color^1, layer - 1);
    ...
    if (!iCanPlay) {
        if (otherCanPlay) {
            solvePair_t ret = teamrand_solveBoard(state, color^1, layer - 1);
            ret.score = 0 - ret.score;
            return ret;
        }
        else {
            int score;
            if (teamrand_count(state, color) > teamrand_count(state, color^1)) score = 1e8;
            else if (teamrand_count(state, color) < teamrand_count(state, color^1)) score = -1e8;
            else score = 0;
            pos_t pos = teamrand_makePos(-1, -1);
            solvePair_t ret = teamrand_makeSolvePair(pos, score);
            return ret;
        }
    }
    ...
    solvePair_t pair = teamrand_makeSolvePair(bestPos, best);
    return pair;
...
board_t teamrand_loadBoard(const enum piece board[][SIZE]) {
    ...
    for (int8_t i = 0; i < 8; i++) {
        for (int8_t j = 0; j < 8; j++) {
            ...
            int8_t ind = teamrand_getIndex(i, j);
            switch (board[i][j]) {
                case EMPTY: break;
                case WHITE: teamrand_setBit(&res.color, ind, 1); // fall through to set res.on bit
                case BLACK: teamrand_setBit(&res.on, ind, 1); // don't need to set color for black
...
void teamrand_print(board_t state) {
    ...
    for (int8_t i = 0; i < 8; i++) {
        ...
        for (int8_t j = 0; j < 8; j++) {
            // Print this cell
            int8_t ind = teamrand_getIndex(i, j);
            if (!teamrand_getBit(state.on, ind)) printf(". "); // empty cell
            else {
                // Non-empty cell
                int color = teamrand_getBit(state.color, ind);
...
int teamrand_getPiece(board_t state, pos_t pos) {
    if (!teamrand_hasPiece(state, pos)) return -1;
    return teamrand_getColor(state, pos);
}
...
int teamrand_hasPiece(board_t state, pos_t pos) {
    int8_t ind = teamrand_getIndexByPos(pos);
    return teamrand_getBit(state.on, ind);
}
...
int teamrand_getColor(board_t state, pos_t pos) {
    int8_t ind = teamrand_getIndexByPos(pos);
    return teamrand_getBit(state.color, ind);
}
...
void teamrand_setPiece(board_t *state, pos_t pos, int color) {
    int8_t ind = teamrand_getIndexByPos(pos);
    teamrand_setBit(&state->on, ind, 1);
    teamrand_setBit(&state->color, ind, color);
}
...
int teamrand_count(board_t state, int color) {
    ...
    return teamrand_popcount(teamrand_getPieces(state, color));
}
...
int teamrand_pieceEquals(board_t state, pos_t pos1, pos_t pos2) {
    int a = teamrand_hasPiece(state, pos1);
    int b = teamrand_hasPiece(state, pos2);
    ...
    return teamrand_getColor(state, pos1)
           == teamrand_getColor(state, pos2);
}
...
int8_t teamrand_getIndexByPos(pos_t pos) {
    return teamrand_getIndex(pos.y, pos.x);
}
...
uint64_t teamrand_getMoveMask(pos_t start, pos_t end) {
    ...
    int8_t u = teamrand_getIndexByPos(start);
    int8_t v = teamrand_getIndexByPos(end);
    
    // Horizontal range
    if (start.y == end.y) {
        // All the bits we need to flip are contiguous
        return teamrand_rangeMask(u, v);
    }
    ...
    if (start.x == end.x) {
        ...
        return mask & teamrand_rangeMask(u, v); // restrict the flipped range
    }
    ...
    if (start.x < end.x) {
        ...
        // Restrict the range
        return mask & teamrand_rangeMask(u, v);
    }
...
board_t teamrand_executeMove(board_t state, pos_t pos, int color) {
    ...
    if (teamrand_hasPiece(state, pos)) return state;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        pos_t dir = dirs[i];
        teamrand_executeMovePartial(&state, pos, color, dir.y, dir.x);
    }
...
//    printf("\ndy = %d, dx = %d, col = %d\n", dy, dx, col);
    ...
    pos_t end = teamrand_makePos(start.y + dy, start.x + dx);
    for (; teamrand_inBounds(end); end.y += dy, end.x += dx) {
        int piece = teamrand_getPiece(*state, end);
//        printf("(y, x) = (%d, %d) -> %d\n", end.y, end.x, piece);
...
//            printf("Setting run (%d, %d) -> (%d, %d)\n", start.y, start.x, end.y, end.x);
            teamrand_setPieces(state, start, end, color);
        }
...
void teamrand_flipPieces(board_t *state, pos_t start, pos_t end) {
    ...
    uint64_t mask = teamrand_getMoveMask(start, end);
 ...
 * position
 */
int8_t teamrand_getIndex(int8_t y, int8_t x) { return y * 8 + x; }
